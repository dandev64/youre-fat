...
export default function SessionCard({ session, date, onDelete }) {
  ...
  return (
    <div className="w-full bg-white rounded-2xl shadow-sm border border-black/5 text-left">
      <button
        onClick={() => navigate(`/day/${dateStr}`)}
        className="w-full p-4 active:scale-[0.98] transition-all text-left"
      >
            ...
            <div className="flex items-center gap-4 flex-wrap">
              {session.duration && session.duration !== '-' && (
                <div className="flex items-center gap-1">
                  <span className="material-symbols-outlined text-on-surface-variant/50" style={{ fontSize: '13px' }}>schedule</span>
              ...
              {session.exercises?.length > 0 && (
                <div className="flex items-center gap-1">
                  <span className="material-symbols-outlined text-on-surface-variant/50" style={{ fontSize: '13px' }}>format_list_numbered</span>
                  <span className="text-[11px] text-on-surface-variant">{session.exercises.length} exercises</span>
                </div>
              )}
            </div>
          </div>

          <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${gradClass} flex items-center justify-center shrink-0`}>
            <span className="material-symbols-outlined text-white/90" style={{ fontSize: '22px', fontVariationSettings: "'FILL' 1" }}>
              {iconName}
            </span>
          </div>
        </div>
      ...
      <div className="flex items-center border-t border-black/5 px-2 py-1.5">
        <button
          onClick={() => navigate(`/day/${dateStr}`)}
          className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-xl text-xs font-semibold text-on-surface-variant active:bg-surface-container transition-colors"
        >
        ...
        <button
          ...
          className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-xl text-xs font-semibold text-error/70 active:bg-error/5 transition-colors"
        >
